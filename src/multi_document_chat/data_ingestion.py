...
class DocumentIngestor:
    ...
    def __init__(self, temp_dir:str = "data/multi_doc_chat", faiss_dir="faiss_index", session_id:str|None=None):
        try:
            self.log = CustomLogger().get_logger(__name__)

            # base directory ( create directory inside the workspace)
            ...
            # session directory (sessionized path), at every run, we create a session directory
    ...
    def ingest_files(self, uploaded_files):
        try:
            ...
            for uploaded_file in uploaded_files:
                ext = Path(uploaded_file.name).suffix.lower() #converting (supported file extension) to lowercase incase of beahviour
                ...
                self.log.info("Files saved for ingestion",
            ...
            self.log.info("All documents loaded", total_docs=len(documents),
                              session_id = self.session_id)
            return self._create_retriever (documents)
        
        except Exception as e:
            self.log.error("Failed to ingest files", error = str(e)) 
            raise DocumentPortalException("Ingesting error in DocumentIngestor", sys)
    ...
    def _create_retriever(self,documents):
        try:
            ...
            self.log.info("FAISS index saved to disk", path= str(self.session_faiss_dir),
                          session_id= self.session_id)
            
            retriever = vectorstore.as_retriever(search_type="similarity", search_kwargs= {"k":5})

            self.log.info("FAISS retriever creaed and ready to use", session_id= self.session_id)
            return retriever

        except Exception as e:
            self.log.error("Failed to create retriever", error = str(e))
            raise DocumentPortalException ("Retrieving error in DocumentIngestor", sys)