...
#     """
#     Simulate uploaded file by reading a PDF file into a BytesIO object
#     """
...
#             self._buffer = file_path.read_bytes() # read the file in a binary object, and then capture in a variable
...
# # this is the path location name (where the FAISS index will available--> Path("faise_index")) 
# # this is a global variable and we created a FAISS path location is a database
...
#         else:
#             # Step 2: Ingest document and create retriever (meaning, upload the file in databse)
...
#         # whatever response I will get after invoke, I will keep it as a variable
...
#     question = "What is the significance of the attention mechanism? can you explain it to me in simple terms"
...
def test_document_ingestion_and_rag():
    try:
        ...
        uploaded_files = [] # means to open the file
        
        for file_path in test_files:
            if Path(file_path).exists():
                uploaded_files.append(open(file_path, "rb")) # files opened at this stage
            else:
                print(f"File does not exist: {file_path}")
                
        if not uploaded_files:
            print("No valid files to upload.")
            sys.exit(1)  #means exit from the system
        ...
        for f in uploaded_files:
            f.close() # After the files have been ingested, we close it
